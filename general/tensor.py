...
class MixTensor:
    ...
    def tensor_dim_slice(tensor, dim, dim_slice):
        return tensor[
            (dim if dim >= 0 else dim + tensor.dim()) * (slice(None),) + (dim_slice,)
        ]
    ...
    def __add__(self, mix_tensor):
        assert self.shape == mix_tensor.shape and type(self) == type(
            mix_tensor
        )  # is same shape mix tensor
...
class BatchListTensor:
    ...
    def __add__(self, bs):
        for k in range(len(self.batches)):
            ...
            if isinstance(self.batches[k], MixTensor) and isinstance(
                bs.batches[k], MixTensor
            ):
                self_k = self.batches[k].to_tensor()
                bs_k = bs.batches[k].to_tensor()
                res = self_k + bs_k
                self.batches[k] = MixTensor.from_tensor(
                    res, self.batches[k].percents, self.batches[k].file_path
                )
            elif isinstance(self.batches[k], torch.Tensor) and isinstance(
                bs.batches[k], torch.Tensor
            ):
                ...
                raise NotImplementedError(
                    f"Not implemented: '{type(self.batches[k])}' + '{type(bs.batches[k])}'."
                )
        return self
...
if __name__ == "__main__":
    ...
    m = MixTensor.from_tensor(
        x, percents={"cuda": 0, "cpu": 0.5, "disk": 0.5}, file_path="test/m.dat"
    )
    m2 = MixTensor.from_tensor(
        x, percents={"cuda": 0, "cpu": 0.5, "disk": 0.5}, file_path="test/m2.dat"
    )
    m = m + m2
